...
static inline HAL_StatusTypeDef INV_MPU_I2C_Write(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data)
{
  ...
  while (ret && retries++ < 4)
  {
    ret = HAL_I2C_Mem_Write(&hi2c1, slave_addr << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, INV_MPU6050_I2C_TIMEOUT);
    if (ret)
    {
      HAL_Delay(50);
    }
...
static inline HAL_StatusTypeDef INV_MPU_I2C_Read(unsigned char slave_addr, unsigned char reg_addr, unsigned char length, unsigned char *data)
{
  ...
  while (ret && retries++ < 4)
  {
    ret = HAL_I2C_Mem_Read(&hi2c1, slave_addr << 1, reg_addr, I2C_MEMADD_SIZE_8BIT, data, length, INV_MPU6050_I2C_TIMEOUT);
    if (ret)
    {
      HAL_Delay(50);
    }
